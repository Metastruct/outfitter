...
function db_init()
	...
	_db = assert(_db:create([[
		...
		:coerce { last_used = tonumber, fileid = tonumber, w = tonumber, file_size = tonumber, h = tonumber, locked = function(
			l) return l == '1' end })
...
function find_purgeable()
	...
	dbg("find_purgeable()")
	local a, b = db:select('*', 'WHERE locked != 1 ORDER BY last_used ASC LIMIT(select max(0,count(*) - %d) from %s)',
		MAX_ENTRIES, db)
...
function record_use(fileid, nolock)
	...
	return db:update("last_used = (cast(strftime('%%s', 'now') as int) - 1477777422)" .. nolock .. " WHERE fileid=%d",
		fileid)
end
...
function data_format(bytes)
	if IsJPG(bytes) then
		return 'jpg'
	elseif IsPNG(bytes) then
		return 'png'
	elseif IsVTF(bytes) then
		return 'vtf'
	end
...
function read_image_dimensions(fh, fmt)
	...
	local w, h
	local ok, t, err2 = xpcall(reader, debug.traceback, fh)
	if not ok then
		return nil, t
	end

	if not t then
		return nil, err2 or "could not parse/read file"
	end
		...
		if frameCount > 18 then
			if not errored then
				...
				if not IKNOWWHATIMDOING then
					ErrorNoHalt("URLImage called every frame, you must keep a reference to the result of URLImage, url=" ..
					tostring(url))
					debug.Trace()
				end
			end
	...
	if lastFrameCalled == fn - 1 then
		...
		if frameCount > 18 then
			if not errored then
				...
				if not IKNOWWHATIMDOING then
					ErrorNoHalt(
					"URLMaterial called every frame, you must keep a reference to the result of URLMaterial, url=" ..
					tostring(url))
					debug.Trace()
				end
			end
