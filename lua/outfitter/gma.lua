...
local function verify_files(meta, collect)
	...
	for _, file_meta in pairs(meta.files) do
		...
		if file_meta.crc and file_meta.crc ~= 0 and tostring(util.CRC(data)) ~= tostring(file_meta.crc) then return false,
				file_meta end
...
local function build(meta, gma, collect, no_crc)
	...
	gma:Write(name) -- Name
	add(#name)
	local desc = (meta.description and util.TableToJSON(meta.description) or '{"type":"model","tags":["fun"],"description":"description"}') ..
	'\0'
	...
	for _, file_meta in pairs(meta.files) do
		...
		if no_crc ~= true and file_meta.crc and file_meta.crc ~= 0 and tostring(util.CRC(data)) ~= tostring(file_meta.crc) then return
			nil, "file corrupted" end
		file_meta.offset = gma:Tell()
		gma:Write(data)
		add(#data)
...
if TEST then
	...
	for _, workshop_id in pairs { 2570101454, 1135026995 } do
		steamworks.DownloadUGC(workshop_id, function(path, gma_fd)
			...
			print("DownloadUGC pre-parse", parsed, parsed.eof, not gma_fd:EndOfFile() and "NOT END OF FILE!!!!!!!!" or "",
				-(gma_fd:Tell() - gma_fd:Size()))
