...
do
	...
	local racial_slur_testers = util.Base64Decode(
	"bmlnZ2VyCmhvbW8KYmliYQpwaWNoa3UKbmVncgpsZXNiYQpwZApqaWQKz4DOv8+Nz4PPhM63CmphcMOzCmNoZWNjCuyVoOyekAptYXJpY2EKY2lwCmZ1ZmEKbXVpc3QKZmF4YQpvw6cK0LPQtdC5CsSRxKk=")
	:Split("\n")
	...
	--global nsfw?
	local nsfw = CreateClientConVar("nsfw", "1", true, true,
		"Allow NSFW content (Steam can still block with text filter)")
	function AllowNSFW()
...
function OnDeathRagdollCreated(rag, pl)
	...
	dbgn(2, "DeathRagdollEnforce", pl, rag, mdl, hasenforced and ("ENFORCED RAG: " .. tostring(mdlr)) or "",
		isplyenforced and "" or ("NOT ENFORCED PLY: " .. tostring(mdlp)))
	...
	if opt ~= 'force' then
		...
		if res then
			...
			if err then
				if res ~= false then
					dbg("MountGMA",
						(("AlreadyMounted(%q) returning error, mounting anyway, info: %s\n"):format(fpath, err)))
				end
			else
				-- not mounted, all ok
			end
